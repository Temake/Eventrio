...
class UserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ('id', 'username', 'email', 'password', 'phone_number', 'profile', 'confirm_password')
        extra_kwargs = {
    ...
    def create(self, validated_data):
        ...
        if not email or not username or not phone_number:
            raise serializers.ValidationError({'info': 'Email, username, and phone number are required.'})

        # Check if email already exists
        if User.objects.filter(email=email).exists():
            raise serializers.ValidationError({'info': 'A user with this email already exists.'})
